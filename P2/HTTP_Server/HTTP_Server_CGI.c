...
uint32_t netCGI_Script (const char *env, char *buf, uint32_t buflen, uint32_t *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 'g':
      ...
      switch (env[2]) {
        case '1':
          sprintf(HOUR, "%02d:%02d:%02d", stimestructure.Hours, stimestructure.Minutes, stimestructure.Seconds);
          len = (uint32_t)sprintf (buf,&env[4], HOUR);
          break;
        case '2':
					sprintf(DATE, "%02d-%02d-%02d" , sdatestructure.Date,sdatestructure.Month, 2000 + sdatestructure.Year);
