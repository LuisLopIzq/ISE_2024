...
uint8_t get_button (void) {
  return ((uint8_t) 1);//Buttons_GetState ());
}
...
//    GLCD_DrawString (x*16U, 8U*24U, buf);
    ...
    sprintf(DisplayLCD.Texto,"%-1s", lcd_text[0]);
		DisplayLCD.Linea=1;
		osMessageQueuePut(mid_MsgQueueLCD,&DisplayLCD,0U,0U);
		//osDelay(3000U);
		

		sprintf(DisplayLCD.Texto,"%-1s", lcd_text[1]);
		DisplayLCD.Linea=2;
		osMessageQueuePut(mid_MsgQueueLCD,&DisplayLCD,0U,0U);
		//osDelay(3000U);
