...
void RTC_CalendarShow(uint8_t *showtime, uint8_t *showdate)
{
  ...
  size=sprintf((char *)showtime, "%02d:%02d:%02d", stimestructure.Hours, stimestructure.Minutes, stimestructure.Seconds);
	Borrar_Linea(1);
  for(int i = 0; i< size; i++){
    symbolToLocalBuffer(1,showtime[i]);
  }
	
  /* Display date Format : mm-dd-yy */
  size=sprintf((char *)showdate, "%02d-%02d-%02d",sdatestructure.Date, sdatestructure.Month, 2000 + sdatestructure.Year);
		Borrar_Linea(2);
  for(int i = 0; i< size; i++){
    symbolToLocalBuffer(2,showdate[i]);
  }
	 LCD_update();
}
